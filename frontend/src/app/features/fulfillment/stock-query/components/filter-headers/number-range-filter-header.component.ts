...
interface NumberRangeFilterHeaderParams extends IHeaderParams {
  onFilterChanged?: (from: string, to: string) => void;
}

@Component({
  selector: 'app-number-range-filter-header',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="number-range-header">
      <div
        class="header-label"
      ...
      >
        <div class="popup-title">Filtrar por rango</div>
  ...
  styles: [
    `
      ...
      .filter-icon-btn.active {
        ...
        background: rgba(249, 124, 6, 0.15);
      }
      .filter-summary {
        font-size: 13px;
        color: #f97c06;
      ...
      /* Popup - fixed to avoid AG Grid overflow:hidden clipping */
      .filter-popup {
        position: fixed;
        z-index: 9999;
        min-width: 200px;
        ...
        padding: 12px;
        font-family: var(--ag-font-family, 'Inter', sans-serif);
      ...
      .popup-input:focus {
        border-color: #f97c06;
        box-shadow: 0 0 0 1px rgba(249, 124, 6, 0.15);
      }
...
export class NumberRangeFilterHeaderComponent
  implements IHeaderAngularComp, OnDestroy
{
  ...
  params!: NumberRangeFilterHeaderParams;
  displayName = '';
  sortState: 'asc' | 'desc' | '' = '';
  showPopup = false;
  hasFilter = false;
  ...
  agInit(params: NumberRangeFilterHeaderParams): void {
    this.params = params;
  ...
  refresh(params: NumberRangeFilterHeaderParams): boolean {
    this.params = params;
    this.displayName = params.displayName || '';
    return true;
  }
  ...
  getFilterSummary(): string {
    const from = this.fromValue !== '' ? this.fromValue : '…';
    const to = this.toValue !== '' ? this.toValue : '…';
    return `${from} — ${to}`;
  }
